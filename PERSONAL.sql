...
SELECT nombre,sal_emp,comision_emp FROM personal.empleados WHERE id_depto=2000;
...
SELECT nombre AS 'NOMBRE', (sal_emp+comision_emp+500) AS 'TOTAL A PAGAR' FROM personal.empleados WHERE id_depto=3000 ORDER BY nombre asc;
/*12. Muestra los empleados cuyo nombre empiece con la letra J.*/
SELECT nombre AS 'NOMBRE' FROM personal.empleados WHERE nombre like 'J%';
...
select nombre,sal_emp,comision_emp, (sal_emp+comision_emp) AS 'TOTAL A PAGAR' FROM personal.empleados where comision_emp>1000;
/*14. Obtener un listado similar al anterior, pero de aquellos empleados que NO
tienen comisión.*/
select nombre,sal_emp,comision_emp, (sal_emp+comision_emp) AS 'TOTAL A PAGAR' FROM personal.empleados where comision_emp=0;

/*15. Obtener la lista de los empleados que ganan una comisión superior a su sueldo.*/
select nombre,sal_emp,comision_emp from personal.empleados where comision_emp>sal_emp;
/*16. Listar los empleados cuya comisión es menor o igual que el 30% de su sueldo.*/
select nombre,sal_emp,comision_emp from personal.empleados where (sal_emp*0.30)>=comision_emp;
...
SELECT distinct nombre_depto FROM personal.departamentos WHERE nombre_depto  IN ('VENTAS','INVESTIGACION','MANTENIMIENTO');

/*19. Ahora obtener los nombres de los departamentos que no sean “Ventas” ni  “Investigación” ni ‘Mantenimiento.*/
SELECT distinct nombre_depto FROM personal.departamentos WHERE nombre_depto  NOT IN ('VENTAS','INVESTIGACION','MANTENIMIENTO');
/*20. Mostrar el salario más alto de la empresa.*/
select nombre,max(sal_emp) AS 'SALARIO' FROM personal.empleados;
/*21. Mostrar el nombre del último empleado de la lista por orden alfabético.*/
SELECT max(nombre) from personal.empleados order by nombre desc;
/*22. Hallar el salario más alto, el más bajo y la diferencia entre ellos.*/
SELECT nombre, max(sal_emp), min(sal_emp),(max(sal_emp)-min(sal_emp)) as 'diferencia' from personal.empleados;
...
SELECT COUNT(id_emp),id_depto AS 'CODIGO DE DEPARATAMENTO'
FROM empleados 
GROUP BY id_depto
...
SELECT E.cod_jefe AS 'CODIGO DE JEFE', E.nombre AS ' NOMBRE', COUNT(*) AS 'CANTI EMPLEADOS'
...
drop database libreriaj;
create database libreriaj;
